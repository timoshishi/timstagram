  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    switch (authView) {
      case 'sign_in':
        ...
        break;
      case 'sign_up':
        const previousUser = supabaseClient.from('profile').select('id').eq('username', username);
        const { data: users, error: usersError } = await previousUser;
        ...
        console.log({ signUpUser, signUpSession, signUpError });
...
function UpdatePassword({ supabaseClient }: { supabaseClient: SupabaseClient }) {
